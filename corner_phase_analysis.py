...
def analyze_mes_corner_phases(
    ...
    for lap_idx, (lap_start, lap_end, lap_t_s) in enumerate(boundaries):
        ...
        for c_idx, apex in enumerate(apex_areas):
            ...
            # brake_peak_progress: position of this apex within the lap (0.0–1.0)
            brake_lap_samples    = b_end - b_start
            apex_brake_start     = apex["start"]
            brake_peak_progress  = round(float(np.clip(
                apex_brake_start / max(brake_lap_samples, 1), 0.0, 1.0
            )), 4)
            ...
            row = {
                ...
                "corner_no":           c_idx + 1,
                "brake_peak_progress": brake_peak_progress,
                "pseudo_corner_id":    None,
                "cluster_confidence":  None,
                "cluster_lap_count":   None,
            }
            row.update(phase)
            results.append(row)
...
# ── pseudo_corner_id クラスタリング ───────────────────────────────────
def assign_pseudo_corner_ids(rows: list) -> list:
    """
    同一 (circuit, rider, session_type, run_no) グループ内で
    brake_peak_progress が近いコーナーをクラスタリングし
    pseudo_corner_id と cluster_confidence を付与する。
    """
    from collections import defaultdict

    PROGRESS_THRESHOLD = 0.020  # ±2%進行率を同一コーナーとみなす
    MIN_LAPS_HIGH      = 5      # 高信頼度に必要な最小ラップ数
    MIN_LAPS_MED       = 2      # 中信頼度の最小ラップ数
    MAX_STD_HIGH       = 0.008  # 高信頼度の最大標準偏差
    MAX_STD_MED        = 0.018  # 中信頼度の最大標準偏差

    groups = defaultdict(list)
    for i, row in enumerate(rows):
        if row.get("brake_peak_progress") is None:
            continue
        key = (row["circuit"], row["rider"], row["session_type"], row["run_no"])
        groups[key].append(i)

    for key, indices in groups.items():
        sorted_indices = sorted(indices, key=lambda i: rows[i]["brake_peak_progress"])

        clusters = []
        for i in sorted_indices:
            prog   = rows[i]["brake_peak_progress"]
            placed = False
            for cluster in clusters:
                c_mean = np.mean([rows[j]["brake_peak_progress"] for j in cluster])
                if abs(prog - c_mean) <= PROGRESS_THRESHOLD:
                    cluster.append(i)
                    placed = True
                    break
            if not placed:
        ...
        clusters.sort(key=lambda c: np.mean([rows[j]["brake_peak_progress"] for j in c]))

        for c_num, cluster in enumerate(clusters, start=1):
            corner_id = f"C{c_num:02d}"
            progs     = [rows[j]["brake_peak_progress"] for j in cluster]
            n_laps    = len(cluster)
            std       = float(np.std(progs)) if len(progs) > 1 else 0.0

            if n_laps >= MIN_LAPS_HIGH and std <= MAX_STD_HIGH:
                confidence = "高"
            elif n_laps >= MIN_LAPS_MED and std <= MAX_STD_MED:
                confidence = "中"
            else:
                confidence = "低"

            for j in cluster:
                rows[j]["pseudo_corner_id"]  = corner_id
                rows[j]["cluster_confidence"] = confidence
                rows[j]["cluster_lap_count"]  = n_laps
...
def main():
    ...
    # ── pseudo_corner_id クラスタリング ───────────────────────────────
    print("  pseudo_corner_id を付与中...")
    all_rows   = assign_pseudo_corner_ids(all_rows)
    high_conf  = sum(1 for r in all_rows if r.get("cluster_confidence") == "高")
    med_conf   = sum(1 for r in all_rows if r.get("cluster_confidence") == "中")
    low_conf   = sum(1 for r in all_rows if r.get("cluster_confidence") == "低")
    print(f"  cluster_confidence: 高={high_conf} 中={med_conf} 低={low_conf}")
