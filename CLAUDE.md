...
| `corner_phase_data.json` | 17387行 | round, circuit, date, session_type, rider, run_no, lap_no, lap_time_s, corner_no, ph12_duration_ms, ph12_brake_peak_bar, ph12_susf_avg, ph3_duration_ms, ph3_speed_min, ph3_susf_avg, ph3_susr_avg, ph45_duration_ms, ph45_gas_avg, ph45_susf_avg, total_corner_ms, **brake_peak_progress** (0.0–1.0 ラップ内位置), **pseudo_corner_id** (C01–C15等 クラスター識別子), **cluster_confidence** (高/中/低), **cluster_lap_count** (クラスター内ラップ数) | corner_phase_analysis.py 実行時 |
...
- **cluster_confidence「低」のコーナーを根拠にした断定的提案** → 信頼度が低いコーナーは物理コーナーとの対応が不安定。「C05は遅い」のような断言はしない。
- **pseudo_corner_id 未付与（None）のコーナーを FAST/SLOW 比較に使用** → corner_no はラップ間で同一コーナーを保証しない。pseudo_corner_id が None のデータはコーナー比較の根拠に使わない。
